...
const btnSort = document.querySelector(".sort-box");
...
function sortTodoList(event) {
  ...
  if (sortType === "Importance") {
    const mapped = todoList.map(function (el, i) {
      return { index: i, importance: el.importance };
    });
    mapped.sort((a, b) => (a.importance < b.importance ? 1 : a.importance > b.importance ? -1 : 0));
    result = mapped.map(function (el) {
      return idList[el.index];
    });
  } else {
    const mapped = todoList.map(function (el, i) {
      return { index: i, dueDate: el.dueDate };
    });
    mapped.sort((a, b) => (a.dueDate > b.dueDate ? 1 : a.dueDate < b.dueDate ? -1 : 0));
    result = mapped.map(function (el) {
      return idList[el.index];
    });
  }
...
function init() {
  ...
  btnSort.addEventListener("click", sortTodoList);
}
